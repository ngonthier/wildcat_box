...
object_categories = ['angel', 'beard','capital','Child_Jesus','crucifixion_of_Jesus','Mary','nudity', 'ruins','Saint_Sebastien','turban']
...
class main_WikiTenLabelsClassification(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None):
        self.root = root
        self.path_devkit = os.path.join(root, 'VOCdevkit')
        self.path_images = '/media/HDD/data/Wikidata_Paintings/WikiTenLabels/JPEGImages/'
        self.set = set
        ...
        file_csv = os.path.join(path_csv, 'WikiTenLabels_Classification_' + set + '.csv')
        ...
        print('[dataset] WikiTenLabels classification set=%s number of classes=%d  number of images=%d' % (
